...
## 商品同定（別の商品を比べないための絞り込み）

キーワード検索の結果をそのまま並べると、あるモールでは本体が、別のモールでは
**互換ケースやアクセサリ**がヒットし、別商品どうしを比較してしまいます。
安いアクセサリがそのまま「最安」として1位に出るため、実害が大きい問題です。

次の4つの手掛かりで、検索意図と一致しない出品を除外します。

| 手掛かり | 内容 |
| --- | --- |
| 型番 | 検索語に型番（`WH-1000XM5` など）があれば、商品名にも同じ型番が必要。全角・半角・ハイフンのゆれは吸収 |
| JANコード | JANが取れた出品は、代表JANと一致するものだけを残す。JANを返さないモールの出品は巻き添えにしない |
| 商品名 | 検索語のトークンが商品名にどれだけ含まれるか（既定しきい値60%） |
| アクセサリ語 | 「ケース」「カバー」「フィルム」等を含む出品を除外。ただし検索語自体がそれを指す場合は除外しない |

除外した出品は**理由つきで一覧表示**されるので、絞り込みすぎていないか確認できます。

さらに、残った出品のうち価格が中央値から大きく外れるものには「要確認」の印を付けます
（**除外はしません**。本当に掘り出し物の可能性があるため）。

絞り込みを止めたい場合は CLI の `--no-filter`、APIの `strict_matching: false` を使います。
しきい値は `--match-threshold 0.4` のように調整できます。
...
カレントディレクトリの `.env` は CLI・Webサーバーの起動時に自動で読み込まれます
（追加ライブラリは不要）。環境変数を直接渡した場合はそちらが優先されます。
APIキーが未設定でもサンプルデータで一通り動作します。

キーが効いているかは `--no-demo` を付けて確認できます。サンプルに紛れず、
取得できなければ0件になります。

```bash
python -m saiyasu "Anker PowerCore 10000" --platforms rakuten --no-demo
```
...
# 商品同定の絞り込みを無効化／しきい値を緩める
python -m saiyasu "ソニー WH-1000XM5" --no-filter
python -m saiyasu "ソニー WH-1000XM5" --match-threshold 0.4
```
...
`jan`（JANコード）と `model_number`（型番）を入れておくと、商品同定の精度が上がります。
...
- 商品同定は商品名・型番・JANコードによる推定です。完全ではないため、購入前に
  各サイトで同一商品かどうか（容量・色・付属品・並行輸入品でないか）をご確認ください。
